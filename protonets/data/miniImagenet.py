...
def convert_tensor(key, d):
    ...
    '''
    d[key] = 1.0 - rgb_to_grayscale(torch.from_numpy(np.array(d[key], np.float32, copy=False)).transpose(0, 1).contiguous().view(3, d[key].size[0], d[key].size[1])) 
    # d[key] = 1.0 - torch.from_numpy(np.array(d[key], np.float32, copy=False)).transpose(0, 1).contiguous()[0].view(1, d[key].size[0], d[key].size[1])
    return d
...
def load_class_images(d):
    if d['class'] not in MINIIMAGENET_CACHE:
        ...
        # print(len(ListDataset(class_images).list))
